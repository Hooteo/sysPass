...
":input[type=password]",function(a){if(13===a.keyCode){a.preventDefault();a=$(this);try{h(a)}catch(c){d.error(c),f.error(c)}a.closest("form").submit()}})},n=function(){d.info("initializeClipboard");if(clipboard.isSupported())$("body").on("click",".clip-pass-button",function(){b.actions.account.copyPass($(this)).done(function(a){if(0!==a.status)return void f.out(a);k.set(a.csrf);clipboard.copy(a.data.accpass).then(function(){f.ok(b.config.LANG[45])},function(a){f.error(b.config.LANG[46])})}).fail(function(){f.error(b.config.LANG[46])})}).on("click",".clip-otp-button",function(){b.actions.account.copyPass($(this)).done(function(a){if(0!==a.status)return void f.out(a);k.set(a.csrf);clipboard.copy(a.data.code).then(function(){f.ok(b.config.LANG[45])},function(a){f.error(b.config.LANG[46])})}).fail(function(){f.error(b.config.LANG[46])})}).on("click",
